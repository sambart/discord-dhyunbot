...
class VoiceGate(Cog):
    ...
    async def _ping_newcomer(self, member: discord.Member) -> None:
        ...
        log.debug("User is not verified. Checking cache.")
        if await self.redis_cache.contains(member.id):
            log.debug("User %s already in cache. Ignore.", member.id)
            return

        log.debug("User %s is unverified and has not been pinged before. Sending ping.", member.id)
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState) -> None:
        ...
        if member.bot:
            log.debug("User %s is a bot. Ignore.", member.id)
            return

        # member.voice will return None if the user is not in a voice channel
        if member.voice is None:
            log.debug("User %s not in a voice channel. Ignore.", member.id)
            return

        if isinstance(after.channel, discord.StageChannel):
            log.debug("User %s joined a stage channel. Ignore.", member.id)
            return
